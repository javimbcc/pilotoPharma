...
public class conexionPostgreSQL {
	...
	public Connection añadirConexionPostgreSql(final String user, final String pass, final String port,
		...
		try {
			...
			/*
			 * Utilizamos el Class.forName para obtener la instancia de la clase java para registrarla como dirver JDBC
			 */
			try {
				Class.forName("org.postgresql.Driver");
			} catch (ClassNotFoundException cnfe) {
				System.out.println(
						"Error en registro driver PostgreSQL: " + cnfe);
			}
			...
			System.out.println(esValida ? "La Conexión a PostgreSQL es válida"
					: "La Conexión a PostgreSQL no es válida");
			return conexion;

		} catch (java.sql.SQLException jsqle) {

			System.out.println(
					"Error en conexión a PostgreSQL (" + url + "): " + jsqle);
			return conexion;
